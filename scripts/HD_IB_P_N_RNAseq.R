...
group[c("RNA_1_S11_R1_001", 
        "RNA_3_S13_R1_001", 
        "RNA_11_S21_R1_001")] <- "IB_Negative"

#Inclusion body positive
group[c("RNA_2_S12_R1_001",
        "RNA_4_S14_R1_001",
        "RNA_12_S22_R1_001")] <- "IB_Positive"

group
group <- factor(group, levels = c("IB_Negative", "IB_Positive"))  # controls as reference
group
...
ggsave(file.path(output_dir, "TE_PCA_IB_Positive_vs_IB_Negative.png"),
...
# coef = 2 corresponds to "IB_Positive" (IB_Positive vs IB_Negative)
...
# Save DEA table
write.csv(res_TE,
          file = file.path(output_dir, "TE_DEA_results_IB_Positive_vs_IB_Negative.csv"),
...
res_TE$direction <- ifelse(
  res_TE$FDR < 0.05 & res_TE$logFC > 0,  "Up in IB_Positive",
  ifelse(res_TE$FDR < 0.05 & res_TE$logFC < 0, "Down in IB_Positive", "Not significant")
)
...
p_volcano <- ggplot(volcano_df, aes(x = logFC, y = neg_logFDR)) +
  ...
  scale_color_manual(
    values = c(
      "Up in IB_Positive"   = "#d73027",  # red
      "Down in IB_Positive" = "#4575b4",  # blue
      "Not significant" = "grey80"
    )
  ) +
  labs(
    title = "Differential Expression — Transposable Elements",
    x = "Log2 Fold Change (IB_Positive / IB_Negative)",
...
ggsave(file.path(output_dir, "TE_volcano_IB_Positive_vs_IB_Negative_labeled.png"),
...
write.csv(gsea_results,
          file = file.path(output_dir, "TE_family_enrichment_IB_Positive_vs_IB_Negative.csv"),
          row.names = FALSE, quote = FALSE)

write.csv(gsea_class,
          file = file.path(output_dir, "TE_class_enrichment_IB_Positive_vs_IB_Negative.csv"),
          row.names = FALSE, quote = FALSE)
...
gsea_results2 <- gsea_results %>%
  ...
  mutate(
    direction = case_when(
      FDR < 0.05 & median_rank > 0 ~ "Enriched UP in IB_Positive",
      FDR < 0.05 & median_rank < 0 ~ "Enriched DOWN in IB_Positive",
...
plot_df <- gsea_results2 %>%
  ...
  mutate(
    ...
    dir_color = case_when(
      direction == "Enriched UP in IB_Positive"   ~ "UP",
      direction == "Enriched DOWN in IB_Positive" ~ "DOWN",
      TRUE                                    ~ "NS"
    )
  )
...
p_enrich <- ggplot(plot_df, aes(x = family, y = -log10(FDR), fill = dir_color)) +
  ...
  scale_fill_manual(values = cols, name = "Direction",
                    labels = c(
                      "UP"   = "Enriched in IB_Positive",
                      "DOWN" = "Depleted in IB_Positive",
                      "NS"   = "Not significant"
                    )) +
  labs(
    x = "TE family",
...
p_enrich
ggsave(file.path(output_dir, "TE_family_enrichment_direction_IB_Positive.png"),
